...
## 🔗 Live Demo
👉 [View Live Demo](https://amn0805.github.io/-ecommerce-store-Amna/)

## 🎥 Video Walkthrough
📽️ [Watch on YouTube](https://youtu.be/fLfB0p0wL1M) 
...
5. [Feature Breakdown & How Each One Works](#feature-breakdown--how-each-one-works)
...
8. [LocalStorage Keys Reference](#localstorage-keys-reference)
...
## Feature Breakdown & How Each One Works
 ...
 ### 1.  Core Product Listing(Data Fetching & API)
...
### 🛒 Cart Drawer Open
![Cart Drawer](./images/cart.png)

---

## Final Thought

Three days ago I could build simple pages. Today I can build a complete, production-style web application — real API data, persistent state, responsive design, multiple features — without a single framework. That feels like a real milestone.

## What I Learned

### State Management

When search, category, and sort all had to work together, I made the mistake of updating the grid from multiple places — and filters kept undoing each other. The fix was simple: one function reads all the state, one function updates the grid. Every user action just updates a variable and triggers that cycle. That one pattern eliminated an entire class of bugs.

### Closures Finally Clicked

I had read about closures many times but never really got them. Writing the debounce function myself changed that. The timerId variable lives privately inside the function — not global, not shared — just quietly remembered between calls. That is exactly what a closure is, and I only understood it by writing it myself.

### LocalStorage Is More Than Storage
I thought localStorage just saved strings. It is actually how you keep state alive across page refreshes. You have to think about when to save, when to load, and how to handle broken data with try/catch. When the cart restored perfectly after a refresh, it felt like real engineering.

### The Dark Mode Flash

My dark mode worked — but every refresh showed a white flash before it applied. The bug was not logic, it was timing. Moving a small script into <head> fixed it, because it runs before the browser paints anything. Some bugs are not about wrong code — they are about code running at the wrong moment.

### The Biggest Challenge — Responsive Header

The header broke me for an entire day. The logo was overlapping the 
navigation, the hamburger button was invisible, and the nav links were 
showing inline on mobile instead of hiding. I tried grid, flexbox, 
absolute positioning — everything seemed to break something else.

The root cause turned out to be two things I had never heard of before:
1. `backdrop-filter` on the header was breaking `position: fixed` children
   on mobile — a browser stacking context issue
2. A malformed CSS comment `} */ */` was silently corrupting everything 
   after it in the file — the parser was ignoring entire media query blocks

This taught me that CSS bugs are not always about wrong values — sometimes
they are about the file itself being broken in a way that gives no error.

### Duplicate CSS — A Silent Killer

At one point I had the same classes defined 2-3 times in my stylesheet 
with conflicting values. `.hero-content`, `.hero-image`, and `@keyframes` 
were all duplicated. The page looked different every time I changed 
something because I was editing the wrong copy.

I learned to always search for a class name before writing it, and to 
keep CSS organized in one place per component. Duplicate code does not 
just waste space — it actively fights against you.

### JavaScript Modules Without a Framework

Splitting JS into 6 separate files (`api.js`, `filters.js`, `cart.js`, 
`product.js`, `ui.js`, `app.js`) felt confusing at first. How does 
`cart.js` use a function from `ui.js`? How does `app.js` know about 
everything?

I learned that when you load scripts in order in HTML, each file's 
functions become available to the files that load after it. This is 
how vanilla JS modules work without `import/export`. It forced me to 
think carefully about dependency order.


### CSS Custom Properties Changed Everything

Using `--clr-accent`, `--clr-navy`, `--clr-surface` throughout the 
entire stylesheet meant that dark mode only needed to redefine these 
variables under `body.dark`. One class change — every color on the 
page updates instantly. I will never hardcode colors in CSS again.

### What I Would Do Differently

Plan the CSS architecture before writing a single line. One section per component, no duplication, and mobile testing from day one — not at the end. Most of my debugging time was spent fixing things that better planning would have prevented.
